 ...
 *
 *   // Replace your exportData function call with:
  ...
  return `
    <div class="gauge-card">
      <svg width="86" height="86" viewBox="0 0 86 86">
        ...
        <text x="43" y="47" text-anchor="middle" font-size="13" font-weight="700" fill="${color}" font-family="Arial">${percent}%</text>
      </svg>
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* HEADER */
  .header {
    background: linear-gradient(135deg, var(--green-dk) 0%, var(--green) 55%, var(--teal) 100%);
  ...
  .header-row { display:flex; justify-content:space-between; align-items:center; margin-top:16px; }
  .export-info { font-size:11px; color:rgba(255,255,255,0.7); }
  ...
  .c1::before { background:var(--green); }
  .c2::before { background:var(--teal); }
  .c3::before { background:var(--gold); }
  .c4::before { background:var(--ruby); }
  .stat-val   { font-size:26px; font-weight:900; line-height:1; margin-bottom:4px; }
  .c1 .stat-val { color:var(--green); }
  .c2 .stat-val { color:var(--teal); }
  .c3 .stat-val { color:var(--gold); }
  .c4 .stat-val { color:var(--ruby); }
  .stat-lbl   { font-size:9px; color:var(--slate-md); font-weight:700; text-transform:uppercase; letter-spacing:0.5px; }
  .stat-sub   { font-size:9px; color:var(--slate-lt); margin-top:4px; }
  ...
  .sec-title {
    ...
    letter-spacing:1.1px; color:var(--green-dk);
  ...
  .dhikr-table td { padding:9px 8px; vertical-align:middle; }
  .td-name  { width:28%; }
  .td-bar   { width:50%; }
  .td-badge { width:14%; text-align:right; white-space:nowrap; }
  .d-name   { font-weight:700; font-size:12px; color:var(--slate); }
  .d-ar     { font-size:10px; color:var(--slate-lt); margin-top:1px; }
  ...
  .sk  { color:var(--slate-md); }
  .sv  { font-weight:700; color:var(--slate); }
  .s-on  { color:var(--green); }
  .s-off { color:var(--ruby); }
  ...
  .footer {
    ...
    font-size:9px; color:var(--slate-lt);
  }
  ...
  <div class="header-inner">
    ...
    <div class="header-row">
      <div class="export-info">
        <strong>${exportDate}</strong>&nbsp;&nbsp;·&nbsp;&nbsp;${exportTime}
      </div>
...
<div class="page">
  ...
  <div class="stats-row">
    <div class="stat-card c1">
      <div class="stat-val">${state.completedWirds.length}</div>
      <div class="stat-lbl">Wirds Done</div>
      <div class="stat-sub">Last: ${lastWird}</div>
    </div>
    <div class="stat-card c2">
      <div class="stat-val">${state.completedWazifas.length}</div>
      <div class="stat-lbl">Wazifas Done</div>
      <div class="stat-sub">Last: ${lastWazifa}</div>
    </div>
    <div class="stat-card c3">
      <div class="stat-val">${state.completedHadras.length}</div>
      <div class="stat-lbl">Hadra Sessions</div>
  ...
  <div class="gauges-row">
    ${circleGauge('Daily Wird',  wirdPct,  '#059669', 'today')}
    ${circleGauge('Wazifa',      wazifaPct,'#0D9488', 'this session')}
    ${circleGauge('Hadra',       hadraPct, '#D97706', 'today')}
  </div>

  <!-- WIRD -->
  <div class="sec-title"><span class="sec-dot"></span>Daily Wird — Today</div>
  <div class="card">
  ...
  <!-- WAZIFA -->
  <div class="sec-title"><span class="sec-dot" style="background:var(--teal)"></span>Weekly Wazīfa — Today</div>
  <div class="card">
    <table class="dhikr-table">
      <tr>
        <td class="td-name"><div class="d-name">Istighfār</div><div class="d-ar">Astaghfiru-llāh</div></td>
  ...
  <!-- HADRA -->
  <div class="sec-title"><span class="sec-dot" style="background:var(--gold)"></span>Ḥaḍra Session — Today</div>
  <div class="card">
    <table class="dhikr-table">
      <tr>
        <td class="td-name"><div class="d-name">Tahlīl</div><div class="d-ar">Lā ilāha illa-llāh</div></td>
...
<div class="page">
  ...
  <div class="sec-title" style="margin-bottom:14px">
    <span class="sec-dot" style="background:var(--slate-md)"></span>
  ...
  <div class="settings-grid">
    ...
    <div class="settings-block">
      <div class="sb-title">Formulas &amp; Targets</div>
...
export const exportReport = async (state: AppState): Promise<void> => {
  try {
    ...
    // 1. Generate PDF (expo-print creates a random temp file)
    const { uri } = await Print.printToFileAsync({ html, base64: false });

    // 2. Rename to a clean, dated filename using the new expo-file-system File API
    ...
    // 3. Check if sharing is supported on this device
    const canShare = await Sharing.isAvailableAsync();

    if (canShare) {
      // Opens native share sheet: Save to Files, AirDrop, WhatsApp, Email…
