...
import {
  ...
  StyleSheet, ScrollView, Pressable, Dimensions,
} from 'react-native';
...
function HadraCard({ item, isActive, onPress }: { item: MenuItem; isActive: boolean; onPress: () => void }) {
...
// ─── Standard card ────────────────────────────────────────────────────────────
function MenuCard({ item, isActive, onPress }: { item: MenuItem; isActive: boolean; onPress: () => void }) {
  const Icon  = item.icon;
              ...
              <View style={s.grid}>
                {group.items.map(item => {
                  ...
                  if (item.route === '/hadra-station')
                    return <HadraCard key={item.route} item={item} isActive={isActive} onPress={navigate} />;

                  return <MenuCard key={item.route} item={item} isActive={isActive} onPress={navigate} />;
                })}
              </View>
            </View>
          ))}
...
const s = StyleSheet.create({
  ...
  // Standard card
  card: {
    width: '47.5%', backgroundColor: '#F8FAFC', borderRadius: 16,
