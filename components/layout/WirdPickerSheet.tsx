...
function PracticeCard({
  ...
  return (
    <Animated.View style={{ opacity: fadeAnim, transform: [{ scale: scaleAnim }] }}>
      <TouchableOpacity
        style={[s.card, { backgroundColor: item.bg, borderColor: item.accent + '28' }]}
        onPress={onPress}
        activeOpacity={0.72}
      >
        {/* Glow background */}
        ...
        {/* Icône */}
        <View style={[s.iconWrap, { backgroundColor: item.accent + '15', borderColor: item.accent + '30' }]}>
        ...
        <View style={s.cardBody}>
          <View style={s.cardTitleRow}>
            ...
            <Text style={[s.cardArabic, { color: item.accent }]}>{item.arabic}</Text>
          </View>
          <Text style={s.cardDesc}>{item.description}</Text>
        </View>

        {/* Flèche */}
        <View style={[s.arrow, { backgroundColor: item.accent + '12' }]}>
          <ChevronRight color={item.accent} size={16} strokeWidth={2.5} />
        </View>

        {/* Grand texte arabe décoratif */}
