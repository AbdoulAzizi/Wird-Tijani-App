...
// ─── Theme — colors, gradients, typography only ───────────────────────────────
...
function formatHijri(d: Date): string {
  try {
    return d.toLocaleDateString('ar-SA-u-ca-islamic', { day: 'numeric', month: 'long', year: 'numeric' });
  } catch { return ''; }
}
      ...
      >
        ...
        <Animated.View style={[ss.inner, { opacity: fadeIn }]}>
          ...
          <View style={ss.topBar}>
            ...
            <TouchableOpacity
              style={ss.iconBtn} onPress={onMenuPress} activeOpacity={0.7}
            ...
            <View style={ss.bellSlot}>
              {showNotification && (
                <TouchableOpacity
                  onPress={onNotificationPress ?? (() => {})} activeOpacity={0.7}
                  hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}
          ...
          <View style={ss.bottomBar}>
            ...
            <View style={ss.datePill}>
              ...
              {showHijriDate && hijriStr
                ? <Text style={ss.hijriDate}>{hijriStr}</Text>
                : null}
            </View>
          </View>
...
const ss = StyleSheet.create({
  ...
  nameText: {
    fontSize: 20, fontWeight: '700', lineHeight: 26,
    color: color.text.gold, ...shadow.textStrong,
  },
  ...
  pageTitle: {
    fontSize: text.size['2xl'], fontWeight: text.weight.extrabold,
    color: color.text.primary, letterSpacing: text.tracking.tight,
    ...shadow.textSoft,
  },
